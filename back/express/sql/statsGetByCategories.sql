...
cat_closure AS (
  -- Every category is a descendant of itself (seed rows)
  SELECT
    c.id AS ancestor_id,
  ...
  -- Walk down the tree: (ancestor -> child of current descendant)
  SELECT
    cc.ancestor_id,
    c.id AS descendant_id
  FROM cat_closure cc
...
exp_t_children AS (
  -- Roll up exp_t through the closure, so each ancestor includes all descendants
  SELECT
    cc.ancestor_id AS category_id,
    et.currency,
    SUM(et.sum) AS sum,
    SUM(et.sum_inusd) AS sum_inusd
  FROM cat_closure cc
  JOIN exp_t et
    ON et.category_id = cc.descendant_id
  GROUP BY 1, 2
),
...
SELECT
    ...
    (
        ...
        jsonb_build_object(
            ...
            'symbol',    cur.symbol,
            'sum',       etc.sum,
            'sum_inUSD', etc.sum_inusd
        )
        ORDER BY cur.name
        )
        FROM exp_t_children etc
        JOIN currencies cur ON cur.name = etc.currency
        WHERE etc.category_id = ct.id
    ),
    '[]'::jsonb
    ) AS sum_total_with_children
